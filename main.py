...
class Node:
    ...
    def __repr__(self):
        res = ""
        res += f'n{self.id} [label="{self.val}"]\n'
...
def read_token():
    ...
    node = Node("NONE", []) if cur_token is None else Node(cur_token.node_str(), [])
...
def expect_keyword(word):
    if cur_token.word != word:
        raise Exception(f"Expected '{word}' keyword\n Got {cur_token}")
    return read_token()
...
def expect_one_of_keywords(words):
    if not is_one_of_keywords(words):
        raise Exception(f"Expected one of '{words}' keywords\n Got {cur_token}")
    return read_token()
...
def parse_filters_citizen(gender, noname):
    name = f"<filters_citizen_{gender}{'_noname' if noname else ''}>"
    if first_order():
        t1 = parse_order()
        return Node(name, [t1])
...
def parse_filters_citizen1(gender, noname):
    name = f"<filters_citizen1_{gender}{'_noname' if noname else ''}>"
    if is_keyword("с"):
        t1 = read_token()
    ...
    if gender == "plur" and is_one_of_keywords(["мужского", "женского"]):
        ...
        t3 = parse_filters_citizen_next(gender, noname)
        return Node(name, [t1, t2, t3])
    raise Exception(f"Failed to match {name}\nGot {cur_token}")
...
def parse_filters_citizen2(noname):
    name = f"filters_citizen2_{'_noname' if noname else ''}"
...
def parse_filters_citizen_next(gender, noname):
    name = f"<filters_citizen_next_{gender}{'_noname' if noname else ''}>"
    if is_keyword(","):
        t1 = read_token()
...
def parse_number_filter():
    ...
    if is_keyword("не"):
        t1 = read_token()
        t2 = expect_one_of_keywords(["больше", "меньше"])
        t3 = expect_number()
    ...
    raise Exception(f"Failed to match <number_filter>\nGot {cur_token}")
...
def parse_asc_desc():
    if is_one_of_keywords(["возрастания", "увеличения", "убывания", "уменьшения"]):
        return Node("<asc_desc>", [read_token()])
    raise Exception(f"Failed to match <asc_desc>\nGot {cur_token}")
